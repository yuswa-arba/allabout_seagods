...
if ($loggedin = logged_in())
{
    ...
    $query_custom_item = mysql_query("SELECT * FROM `custom_item` 
        WHERE `id_member` = '" . $loggedin["id_member"] . "'
        AND `status` = 'process'
        AND `level` = '0' LIMIT 0,1;");
    $row_custom_item = mysql_fetch_array($query_custom_item);
    $id_custom_item = $row_custom_item['id_custom_item'];

    if ($cart = isset($_POST["submit"]) ? $_POST["submit"] : "")
    {
        if ($cart == "Add to Collection")
        {
            // Data Update Cart
            ...
            if (mysql_num_rows($query_custom_item) == 0)
            {
            ...
            else
            {
                ...
                $query_custom_collection = mysql_query("INSERT INTO `custom_collection`
                    (`id_custom_collection`, `code`, `id_member`, `gender`, `wet_suit_type`, `arm_zipper`, `ankle_zipper`, `image`, `status`, `date_add`, `date_upd`, `level`)
                    VALUES ('', '" . $row_custom_item["code"] . "', '" . $loggedin["id_member"] . "', '$gender', '$wetsuit', '$armZipper', '$ankleZipper', '" . $row_custom_item["image"] . "', 'saved', NOW(), NOW(), '0');");
                ...
                $query_custom_collection_select = mysql_query("SELECT * FROM `custom_collection`
                    WHERE `id_member` = '" . $loggedin["id_member"] . "'
                    AND `code` = '" . $row_custom_item["code"] . "'
                    AND `level` = '0'
                    LIMIT 0,1;");
                $row_custom_collection = mysql_fetch_array($query_custom_collection_select);

                $query_custom_measure = mysql_query("INSERT INTO `custom_measure`
                    (`id_measure`, `id_custom_collection`, `id_member`, `total_body_height`, `head`, `neck`, `bust_chest`, `waist`, `stomach`, `abdomen`, `hip`, `shoulder`, `shoulder_elbow`, `shoulder_wrist`, `arm_hole`, 
                ...
                $query_custom_item_update = mysql_query("UPDATE `custom_item` SET `status` = 'saved', `date_upd` = NOW()
                    WHERE `id_custom_item` = '" . $row_custom_item["id_custom_item"] . "';");
                if (!$query_custom_item_update)
                {
                    echo "<script language='JavaScript'>
                        alert('Unable to update custom item.!');
                        history.back(-1);
                    </script>";
                }

                echo "<script language='JavaScript'>
                    alert('Succesfully adding to your custom collection');
                    location.href = '../member/custommade.php';
                  </script>";
            }
        }
    }
            ...
            function saveViaAJAX() {
                ...
                if (selectValue == 'male' || selectValue == 'female' || selectValue == 'unisex') {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'save.php',
                        data: {action: $('#save_project').val(), canvasData: canvasData},
                        dataType: 'json',
                        success: function (data) {
                            window.alert("Image has been saved");
                            $('#save_project').val(data.id);
                        }
                    });
                }
            ...
            <main style="margin-bottom: 200px; min-height: 330px;">
                <div class="container">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8 col-sm-2 custom-box">
                                ...
                                <div class="text-center form-group">
                                    <button type="button" data-target="#measurement" data-toggle="modal" class="btn btn-default">
                  ...
                  Login First
                                    ...
                                    {
                                        echo '
                  <button  type="button" class="btn btn-default"  name="id_custom_item"
                        onclick="saveViaAJAX();" id="save_project" value="' . ($id_custom_item ?: '') . '">Save Project</button>
                  <button type="submit" class="btn btn-primary" name="submit" value="Add to Collection">Add to Project</button>';
                                    }
                                    ?>
                                </div>
                            </div>
