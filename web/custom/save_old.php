...
if (isset($_POST['action']) && isset($_POST['canvasData'])) {
    ...
    $query_custom_collection = mysql_query("INSERT INTO `custom_collection`
                    (`code`, `id_member`, `gender`, `wet_suit_type`, `arm_zipper`, `ankle_zipper`, `image`, `status`, `date_add`, `date_upd`, `level`)
                    VALUES ('$custom_item_number', '" . $loggedin["id_member"] . "', '$gender', '$wetsuit', '$armZipper', '$ankleZipper', '" . $row_custom_item["image"] . "', 'saved', NOW(), NOW(), '0');");
