...
function admin_template($content = "", $titlebar = "", $titlepage = "", $user = "", $menu = "", $plugin = "", $loggedin = false)
{
    ...
    function show_title_menu($text, $limit)
    {
    	...
    	<div class="sidebar four columns">
                    <div class="widget-area clearfix">
                        <aside class="widget woocommerce widget_product_categories">
                            <h3>TOP ARTICLES</h3>
                            <ul id="menu-main-menu" class="menu menu-main">
                                <li><a href="blog.php"><span><strong>SHOW ALL ARTICLE</strong></span></a></li>';

    $query_blog = mysql_query("SELECT * FROM `blog` WHERE `level` = '0' ORDER BY `reader` DESC LIMIT 0,20;");

    while ($row_blog = mysql_fetch_array($query_blog)) {
        $template .= '<li><a href="read_blog.php?blog=' . $row_blog["id_blog"] . '&ttle=' . $row_blog["title"] . '"><span>' . show_title_menu($row_blog["title"], 35) . '</span></a></li>';
    }
